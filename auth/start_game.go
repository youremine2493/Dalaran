...
import (
	...
	"github.com/youremine2493/Dalaran/npc"
	"github.com/youremine2493/Dalaran/player"
...
func exploreMobs(s *database.Socket) {
	...
	if c.IsDungeon {
		if funk.Contains(database.DungeonZones, c.Map) { //len(DungeonsByMap[ai.Server][ai.Map])
			if database.DungeonsByMap[c.Socket.User.ConnectedServer][c.Map] == 0 {
				switch s.Character.DungeonLevel {
				case 1:
					if s.Character.GeneratedNumber == 0 || s.Character.CanTip == 1 {
						//	if s.Character.PartyID != "" {
						//		party := database.FindParty(s.Character)
						//		if party.Leader.ID == s.Character.ID {
					...
					if s.Character.CanTip == 3 {
						s.Character.CanTip = 1
					}
				case 2:
					dungeon.BossSpawn(60003, s.Character.Socket.User.ConnectedServer)
					/*	if s.Character.PartyID != "" {
						party := database.FindParty(s.Character)
				...
				case 3:
					...
					s.Conn.Write(messaging.InfoMessage(fmt.Sprintf("Congratulations, you reign supreme! Claim your prize. Teleporting to safe zone after 10 secs.")))
					c.IsDungeon = false
					s.Character.DungeonLevel = 1
					time.AfterFunc(time.Second*10, func() {
						...
						s.Character.Socket.User.ConnectedServer = 1
						gomap, _ := s.Character.ChangeMap(1, nil)
						s.Conn.Write(gomap)
					})
				}
			...
			s.Conn.Write(messaging.InfoMessage(fmt.Sprintf("You teleported out of the dungeon")))
			s.Character.IsDungeon = false
			s.Character.DungeonLevel = 1
			dungeon.DeleteMobs(s.User.ConnectedServer)
