...
import (
	...
	"github.com/youremine2493/Dalaran/nats"
...
func (c *Character) ShowItems() ([]byte, error) {
	...
	index := 12
	resp.Insert(utils.IntToBytes(uint64(helmID), 4, true), index) // helm id
	index += 4
	...
	resp.Insert(utils.IntToBytes(uint64(maskID), 4, true), index) // mask id
	index += 4
	...
	resp.Insert(utils.IntToBytes(uint64(armorID), 4, true), index) // armor id
	index += 4
	...
	if weapon1.ItemID > 0 {
		resp.Insert(utils.IntToBytes(uint64(weapon1ID), 4, true), index) // weapon1 id
		index += 4
	...
	if weapon2.ItemID > 0 {
		resp.Insert(utils.IntToBytes(uint64(weapon2ID), 4, true), index) // weapon2 id
		index += 4
	...
	resp.Insert(utils.IntToBytes(uint64(bootsID), 4, true), index) // boots id
	index += 4
...
func (c *Character) BSUpgrade(slotID int64, stones []*InventorySlot, luck, protection *InventorySlot, stoneSlots []int64, luckSlot, protectionSlot int64) ([]byte, error) {
	...
	typeMatch := (stoneInfo.Type == 190 && itemType == PET_ITEM_TYPE) || (stoneInfo.Type == 191 && itemType == HT_ARMOR_TYPE) ||
		(stoneInfo.Type == 192 && (itemType == ACC_TYPE || itemType == MASTER_HT_ACC)) || (stoneInfo.Type == 194 && itemType == WEAPON_TYPE) || (stoneInfo.Type == 195 && itemType == ARMOR_TYPE)
...
func (c *Character) CastSkill(attackCounter, skillID, targetID int, cX, cY, cZ float64) ([]byte, error) {
	...
	if target := skillInfo.Target; target == 0 || target == 2 { // buff skill
		...
		if infection.ID == 99 {
			percent := character.Socket.Stats.ArtsDEF / 10
			infection.ArtsDEF = int(percent)
		}
		if infection.ID == 19 || infection.ID == 23 || infection.ID == 25 || infection.ID == 27 || infection.ID == 29 {
			percent := (character.Socket.Stats.ArtsDEF / 100) * 25
			infection.ArtsDEF = int(percent)
		...
		if err != nil {
			...
			id := buff.ID
			if id == 19 || id == 23 || id == 25 || id == 27 || id == 29 {
				stat := c.Socket.Stats
				stat.ArtsDEF -= infection.ArtsDEF
				infection.ArtsDEF = 0
				infection.AdditionalArtsDEF = 0
				percent := (stat.ArtsDEF / 100) * 25
				infection.ArtsDEF = percent
			} else if id == 99 {
				stat := c.Socket.Stats
				stat.ArtsDEF -= infection.ArtsDEF
				infection.ArtsDEF = 0
			...
			buff = &Buff{ID: id, CharacterID: character.ID, StartedAt: character.Epoch, Duration: int64(duration), Name: skillInfo.Name,
				...
				MaxHP: infection.MaxHP, ParalysisDEF: infection.ParalysisDef, PoisonDEF: infection.ParalysisDef, STR: infection.STR, Accuracy: infection.Accuracy * int(plus), Dodge: infection.DodgeRate * int(plus)}
			buff.Update()
