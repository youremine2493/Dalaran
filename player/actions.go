...
func (h *TravelToCastleHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	if s.Character.Map == 233 {
		...
		index += 5
		resp.Insert([]byte{0x0d, 0x41, 0x73, 0x63, 0x65, 0x6e, 0x73, 0x69, 0x6f, 0x6e, 0x20, 0x53, 0x6b, 0x79}, index) //KLÁN 1
		index += 14
		resp.Insert([]byte{0x02, 0xeb, 0x00, 0x00, 0x00}, index)
		index += 5
		resp.Insert([]byte{0x0d, 0x41, 0x73, 0x63, 0x65, 0x6e, 0x73, 0x69, 0x6f, 0x6e, 0x20, 0x53, 0x6b, 0x79}, index) //KLÁN 2
		index += 14
		resp.Insert([]byte{0x03, 0x5d, 0x06, 0x00, 0x00}, index)
		index += 5
		resp.Insert([]byte{0x0d, 0x41, 0x73, 0x63, 0x65, 0x6e, 0x73, 0x69, 0x6f, 0x6e, 0x20, 0x53, 0x6b, 0x79}, index) //KLÁN 3
		index += 14
		resp.Insert([]byte{0x04, 0xf0, 0x06, 0x00, 0x00}, index)
		index += 5
		resp.Insert([]byte{0x0d, 0x41, 0x73, 0x63, 0x65, 0x6e, 0x73, 0x69, 0x6f, 0x6e, 0x20, 0x53, 0x6b, 0x79}, index) //KLÁN 4
		index += 14
		resp.Insert([]byte{0x05, 0xd7, 0x05, 0x00, 0x00}, index)
		index += 5
		resp.Insert([]byte{0x0d, 0x41, 0x73, 0x63, 0x65, 0x6e, 0x73, 0x69, 0x6f, 0x6e, 0x20, 0x53, 0x6b, 0x79}, index) //KLÁN 5
		index += 14
		return resp, nil
	}
